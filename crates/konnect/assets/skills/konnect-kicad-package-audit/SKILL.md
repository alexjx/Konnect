...
Report generation is read-only with respect to KiCad design sources. For an
authorized correction, first inspect the exact project and audited references,
then state the proposed targets and values before using a raw Konnect mutator:

- use `batch_edit_schematic_components` for supported existing schematic Value,
  Footprint, or custom-field edits; the upstream integration baseline does not
  expose a guarded BOM/DNP mutation, so report that gap; or
- use `set_component_placements` for complete absolute live-PCB footprint
  transforms, followed by readback and the applicable placement/DRC checks.

Load only the required toolsets. Constrain each call to audited references and
retain the exact response. Stop on any rejection, error, or ambiguous outcome;
inspect observed state before retrying. If a required mutation is unavailable
or unauthorized, stop the whole correction unless the user explicitly accepts
partial completion.
