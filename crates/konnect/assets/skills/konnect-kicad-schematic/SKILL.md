...
- Keep protection-domain names semantic without creating net-name aliases. The raw cell negative (`B-`/`BAT-`) remains a distinct named net on the unprotected side of the shunt or protection switch. Name the protected side electrically `GND` from the protection boundary onward.
- Use `P-`/`PACK-` only as a connector pin name, terminal legend, or plain schematic note meaning “protected pack negative”; never place it as a second electrical label on a node already named `GND`. Do not add a resistor, jumper, net-tie, or duplicate label merely to express this naming transition.
- Before validation, enumerate ground-related labels on every changed page. Reject downstream `P-`/`PACK-`, raw `B-`/`BAT-` used as circuit ground, and any intentional `P-`/`GND` dual-label alias. ERC should not retain a `multiple_net_names` waiver for this convention.
