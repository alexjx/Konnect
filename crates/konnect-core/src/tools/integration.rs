    ...
    vec![
        ...
        tool!(
            ...
            json!({
                "type": "object",
                "properties": {
                    "dsn": { "type": "string", "description": "Existing Specctra .dsn input" },
                    "ses_output": { "type": "string", "description": "New .ses output path; existing files are never replaced" },
...
async fn handle_route_specctra_dsn(
    ...
    let dsn = get_path(args, "dsn")?;
    let ses_output = get_path(args, "ses_output")?;
